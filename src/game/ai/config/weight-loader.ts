...
/**
 * Cached configuration for weight loading
 */
let cachedConfig: WeightConfiguration | null = null;

/**
 * Load and parse YAML configuration
 * @returns Promise resolving to the loaded configuration or error
 */
export async function loadConfiguration(): Promise<LoadResult<WeightConfiguration>> {
  try {
    ...
    const validationResult = validateSchema(config);
 ...
 */
export async function getDellacherieWeights(): Promise<LoadResult<EvaluationWeights>> {
 ...
 */
export async function getPhaseWeights(phase: GamePhase): Promise<LoadResult<EvaluationWeights>> {
  const configResult = await loadConfiguration();
  if (!configResult.success) {
    return configResult;
  }
 ...
 */
export async function getAllPhaseWeights(): Promise<
 ...
 */
export async function getAdjustmentMultipliers(
 ...
 */
function validateSchema(config: unknown): LoadResult<WeightConfiguration> {
  try {
    ...
    if (config.schemaVersion !== CURRENT_SCHEMA_VERSION) {
      ...
      if (isCompatibleVersion(config.schemaVersion)) {
        console.warn(
    ...
    const weightValidation = validateWeightStructure(config.evaluators);
 ...
 * Type guard for configuration structure
 */
function isValidConfiguration(config: unknown): config is WeightConfiguration {
 ...
 */
function validateWeightStructure(evaluators: unknown): LoadResult<void> {
  ...
  if (!isValidEvaluationWeights(evaluatorConfig.dellacherie)) {
    return {
      success: false,
  ...
  for (const phase of phases) {
    if (!isValidEvaluationWeights(phaseWeights[phase])) {
      return {
        success: false,
 ...
 */
function isValidEvaluationWeights(weights: unknown): weights is EvaluationWeights {
 ...
 */
function getDefaultConfiguration(): WeightConfiguration {
  return {
 ...
 */
export function getCachedConfiguration(): WeightConfiguration | null {
  return cachedConfig;
}
